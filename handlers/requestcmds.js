...
module.exports = async (client, message) => {
  try{
    if(message.author.id === client.user.id) {
      try{
        await delay(4000);
        if(message) message.delete().catch(e=>console.log("couldn't delete message this is a catch to prevent a crash".grey));
      }catch{ /* */}
    }
    else {
      try{
    ...
    */
    ...
    if (!channel) return message.channel.send({embeds: [new MessageEmbed().setColor(ee.wrongcolor).setTitle("You need to join a voice channel.")]});
    ...
    if (player && channel.id !== player.voiceChannel) return message.channel.send({embeds: [new MessageEmbed().setColor(ee.wrongcolor).setTitle("❌ ERROR | I am already playing somewhere else!").setDescription(`You can listen to me in: \`${message.guild.channels.cache.get(player.VoiceChannel).name}\``)]});
    //if the user is not in the channel as in the db voice channel return error
    if (channel.id !== db.voicechannel) return message.channel.send({embeds: [new MessageEmbed().setColor(ee.wrongcolor).setTitle(`You need to be in the: \`${message.guild.channels.cache.get(db.voicechannel).name}\` VoiceChannel`)]});
    ...
    if (cmd.length === 0) return message.channel.send({embeds: [new Discord.MessageEmbed()
