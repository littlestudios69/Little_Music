                 ...
                 if(failed)
                  ...
                  collector.on(`collect`, async (reaction, user) => {
                    try{
                      ...
                      if (!channel) return message.channel.send({embeds: [new MessageEmbed().setColor(ee.wrongcolor).setFooter(ee.footertext, ee.footericon).setTitle(`${emoji.msg.ERROR} ERROR | You need to join a voice channel.`)]});
                      ...
                      if (player && channel.id !== player.voiceChannel) return message.channel.send({embeds: [new MessageEmbed().setColor(ee.wrongcolor).setFooter(ee.footertext, ee.footericon).setTitle(`${emoji.msg.ERROR} ERROR | I am already playing somewhere else!`).setDescription(`You can listen to me in: ${message.guild.channels.cache.get(player.VoiceChannel).name}`)]});
                      ...
                      case String(emoji.react.show_queue):
                        ...
                        if (!tracks.length)
                          return message.channel.send({embeds: [embed.setDescription(`${emoji.msg.ERROR} No tracks in the queue`)]}).then(msg => {
                          try{
                           msg.delete({timeout: 4000}).catch(e=>console.log("couldn't delete message this is a catch to prevent a crash".grey));
                          }catch{ /* */ }
                        });
                        ...
                        if(tracks.length < 15)
                          return message.channel.send({embeds: [embed.setDescription(tracks.map((track, i) => `**${++i})** [${track.title.substr(0, 35)}](${track.uri}) - ${track.isStream ? "LIVE STREAM" : format(track.duration).split(" | ")[0]} - **requested by: ${track.requester.tag}**`).join("\n"))]}).then(msg => {
                          try{
                           msg.delete({timeout: 4000}).catch(e=>console.log("couldn't delete message this is a catch to prevent a crash".grey));
                          }catch{ /* */ }
                        })
                        ...
                        for(let i = 0; i < limit; i++){
                          await user.send({embeds: [embed.setDescription(String(quelist[i]).substr(0, 2048))]});
                        }
                      ...
                      player = client.manager.players.get(player.guild);
                        if (player.queue.size === 0) {
                            ...
                            if(player.get(`afk-${player.get("playerauthor")}`) || player.get(`afk-${player.guild}`))
                              return client.channels.cache.get(player.textChannel).send({embeds: [embed.setDescription(`I will not Leave the Channel, cause afk is ✔️ Enabled`)]}).then(msg => {
                              try{
                               msg.delete({timeout: 4000}).catch(e=>console.log("couldn't delete message this is a catch to prevent a crash".grey));
                              }catch{ /* */ }
                            });
