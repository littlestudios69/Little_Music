...
module.exports = {
    ...
    try{
      ...
      if(player && channel.id !== player.voiceChannel)
        ...
        try{embed.addField(`${emoji.msg.equalizer} Equalizer: `, `${emoji.msg.playing} Music`, true)}catch{}
        try{embed.addField(`${player.queueRepeat ? `${emoji.msg.autoplay_mode} Queue Loop: ` : `${emoji.msg.autoplay_mode} Song Loop: `}`, `${player.queueRepeat ? `${emoji.msg.enabled} Enabled` : player.trackRepeat ? `${emoji.msg.enabled} Enabled` : `${emoji.msg.disabled} Disabled`}`, true)}catch{}
        ...
        try{embed.addField(`${emoji.msg.repeat_mode} Leave on Empty Queue: `, `${config.settings.LeaveOnEmpty_Queue.enabled ? `${emoji.msg.enabled} Enabled` : `${emoji.msg.disabled} Disabled`}`, true)}catch{}
        ...
        try{embed.addField(`${emoji.msg.premium} 24/7 AFK Setup`, `PLAYER: ${player.get(`afk-${player.get(`playerauthor`)}`) ? `${emoji.msg.enabled} Enabled` : `${emoji.msg.disabled} Disabled`}\nGUILD: ${player.get(`afk-${player.guild}`) ? `${emoji.msg.enabled} Enabled` : `${emoji.msg.disabled} Disabled`}`, true)}catch{}
        ...
        return message.channel.send({embeds: [new MessageEmbed()
            .setColor(ee.wrongcolor)
						.setFooter(ee.footertext, ee.footericon)
            .setTitle(`${emoji.msg.disabled} ERROR | An error occurred`)
