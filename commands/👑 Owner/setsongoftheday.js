          ...
          if(!res.tracks[0])
			...
			.setDescription(`Please retry!`)]}
              ...
              fs.writeFile(`./botconfig/songoftheday.json`, JSON.stringify(ss, null, 3), (e) => {
                  if (e) {
                    console.log(String(e.stack).red);
                    return message.channel.send({embeds: [new MessageEmbed().setFooter(ee.footertext,ee.footericon).setColor(ee.wrongcolor).setTitle(`${emoji.msg.ERROR}  ERROR Writing the File`).setDescription(`\`\`\`${e.message}\`\`\``)]})
                  }
			  ...
			  return message.channel.send({embeds: [new MessageEmbed().setFooter(ee.footertext,ee.footericon).setColor(ee.color).setTitle(`${emoji.msg.SUCCESS}  Successfully set the new Track of the Day`)]})
                });
